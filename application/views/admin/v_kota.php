   ...
   <script>
   	...
   	$(document).ready(function() {
   		...
   		table = $('#dataTable').DataTable({
   			...
   			"ajax": {
   				"url": "<?php echo site_url('kota/datatable'); ?>",
   				"type": "POST"
   			},
   			"columnDefs": [{
   				"targets": [-1],
   	...
   	$('body').on('click', '.delete-kota', function() {
   		let id = $(this).data('id');
   		swal({
   			title: "Yakin",
   			text: "hapus Data?",
   			...
   			if (hapus) {
   				$.ajax({
   					'url': "<?php echo base_url('kota/deleteKota/'); ?>" + id,
   					'method': 'DELETE',
   					'success': function(data) {
   						if (data == 'success') {
   							swal('Berhasil Hapus Kota', {
   								icon: "success"
   							}).then(() => {
   								table.draw();
   							});
   	...
   	$('body').on('click', '.edit-kota', function() {
   		let id = $(this).data('id');
   		$('#modal-kota').modal('show');
   		$('.btn-save').val('edit');
   		$('.modal-title').html('Edit Data Kota');
   		$.ajax({
   			url: '<?= base_url("kota/editKota/") ?>' + id,
   			method: 'GET',
   			dataType: 'json',
   			success: function(data) {
   				$.each(data, function(i, d) {
   					$("[name='id_kota']").val(d.id);
   					$('[name="nama_kota"]').val(d.nama_kota);
   	...
   	$('.btn-save').click(function(e) {
   		...
   		let url;
   		if (check == 'tambah') {
   			url = '<?= base_url("kota/storeKota") ?>'
   		} else {
   			url = '<?= base_url("kota/updateKota") ?>'
   		}
   		let data = $('#form-kota').serializeArray();
